...
Extrae información del configPath y construye URLs raw para cada archivo.
"""
...
def extract_env_from_kubernetes_deployment(doc):
    ...
    def find_kubernetes_env(obj):
        if isinstance(obj, dict):
            # Buscar kubernetes -> deployment -> env
...
def main():
    ...
    try:
        ...
        # ========== Descargar config temporalmente ==========
        print("\n📥 Descargando archivo de configuración...")
        print("   (Se necesita owner/repo temporal solo para leer el config)")
        temp_owner = input("   Owner temporal: ").strip()
        temp_repo = input("   Repo temporal: ").strip()
        
        # Construir URL temporal (asumiendo github.com o mismo server)
        temp_server = "https://alm-github.systems.uk.hsbc"
        temp_url = f"{temp_server}/raw/{temp_owner}/{temp_repo}/refs/heads/{branch}/{config_path}"
        
        config_content = download_file(temp_url, token)
        
        # ========== Parsear configPath y paths ==========
        config_url, paths_templates = parse_config_content(config_content)
        repo_info = parse_github_url(config_url)
        
        print(f"\n✅ Configuración cargada")
        print(f"\n📊 Información del Repositorio (de configPath):")
